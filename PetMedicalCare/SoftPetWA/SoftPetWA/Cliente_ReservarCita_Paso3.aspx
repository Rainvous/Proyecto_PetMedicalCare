...
<asp:Content ID="Content3" ContentPlaceHolderID="cphHead" runat="server">
    <%-- Reutilizamos el CSS del Paso 1 y añadimos el del calendario --%>
            ...
            <div class="row">
                ...
                <div class="col-md-7">
                    ...
                    <div class="calendar-container">
                        <%-- (Aquí va todo tu código del calendario, no necesita cambios) --%>
...
<asp:Content ID="Content4" ContentPlaceHolderID="cphScripts" runat="server">
    <%-- Script para simular la selección visual de fecha y hora --%>
    <script type="text/javascript">
        $(document).ready(function () {
            // Cuando se hace clic en una celda de fecha (que no sea de otro mes)
            $('.date-cell:not(.other-month)').click(function () {
                $('.date-cell').removeClass('selected'); // Quita selección de todas
                $(this).addClass('selected'); // Marca la clicada
                // Aquí podrías hacer una llamada AJAX para cargar los horarios de ese día
                // Por ahora, solo actualiza el texto de ejemplo
                var day = $(this).data('date');
                $('#<%= lblHorariosDisponibles.ClientID %>').text('Horarios Disponibles - ' + day + ' de Octubre');
                // Simular que algunos horarios cambian (solo visual)
                $('.time-slot-btn').removeClass('selected disabled');
                // Ejemplo: deshabilitar 09:00 AM para el día 16
                if (day == 16) {
                    $('.time-slot-btn:contains("09:00 AM")').addClass('disabled').removeClass('selected');
                }
            });

            // Cuando se hace clic en un botón de hora (que no esté deshabilitado)
